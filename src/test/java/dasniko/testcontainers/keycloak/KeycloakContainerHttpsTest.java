...
public class KeycloakContainerHttpsTest {
    ...
    public void shouldStartKeycloakWithTlsSupport() {
        try (KeycloakContainer keycloak = new KeycloakContainer(KC_IMAGE).useTls()) {
            keycloak.start();
    ...
    public void shouldStartKeycloakWithCustomTlsCertAndKey() {
        try (KeycloakContainer keycloak = new KeycloakContainer(KC_IMAGE).useTls("keycloak.crt", "keycloak.key")) {
            keycloak.start();
    ...
    public void shouldStartKeycloakWithCustomTlsKeystore() {
        try (KeycloakContainer keycloak = new KeycloakContainer(KC_IMAGE).useTlsKeystore("keycloak.jks", "keycloak")) {
            keycloak.start();
            checkTls(keycloak, "keycloak.jks", "keycloak");
        }
    }
    ...
    public void shouldThrowNullPointerExceptionUponNullTlsTrustCertFilename() {
        assertThrows(NullPointerException.class, () -> new KeycloakContainer(KC_IMAGE).withTrustedCertificates(null));
    }

    @Test
    @SuppressWarnings("resource")
    public void shouldThrowNullPointerExceptionUponNullHttpsClientAuth() {
        assertThrows(NullPointerException.class, () -> new KeycloakContainer(KC_IMAGE).withHttpsClientAuth(null));
    }

    @Test
    @SuppressWarnings("resource")
    public void shouldThrowNullPointerExceptionUponNullTlsCertificateKeyFilename() {
        assertThrows(NullPointerException.class, () -> new KeycloakContainer(KC_IMAGE).useTls("keycloak.crt", null));
    }

    @Test
    @SuppressWarnings("resource")
    public void shouldThrowNullPointerExceptionUponNullTlsKeystore() {
        assertThrows(NullPointerException.class, () -> new KeycloakContainer(KC_IMAGE).useTlsKeystore("keycloak.jks", null));
    }
